...
if redis.call('GET', KEYS[2]) == nil then
    ...
    sequence = tonumber(redis.call('INCRBY', KEYS[2], tonumber(ARGV[5])));
end
for i = 1, tonumber(ARGV[5]) do
    reslut[i] = tostring(ARGV[1]) .. tostring(ARGV[2]) .. string.format(tostring(ARGV[4]), (sequence - tonumber(ARGV[5]) + i));
end
return table.concat(reslut, ',');
